    ...
    xhr.onload=function(){
        if(status.includes(xhr.status)){
            ...
            $addcar.on('click',function(){
                ...
                var status=[304,200]
                var $name=encodeURI(curent_data.name);
                var $l_price=encodeURI(curent_data.l_price);
                var $f_price=encodeURI(curent_data.f_price);
                var xhr=new XMLHttpRequest();
                xhr.onload=function(){
                    if(status.includes(xhr.status)){
                ...
                var geturl='../api/shoppingcar.php?username=null&size='+curent_data.size+'&l_price='+$l_price+'&f_price='+$f_price+'&name='+ $name+'&index='+curent_data.id;
